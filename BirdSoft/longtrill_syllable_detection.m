function [detect , segs , info] = longtrill_syllable_detection(xx , fs , yin , harmonics , ax , f0band)
...
% % tbulk = yin.time([istart , iend]);
% % persegs = persegs(:,persegs(1,:)>tbulk(1) & persegs(2,:)<tbulk(2));
% % 
% evad = energy_vad(xx , fs , 0.01 , 0.1 , 1000 , yin.time(istart:iend));
% tmp = zeros(size(yin.time));
% tmp(istart:iend) = evad;
% evad = tmp;
% testperenergytime = logical2segments(medfilt1(double(periodictimeyin & evad),3), 1/(yin.time(2)-yin.time(1)));
% % testperenergytime = segments2logical(testperenergytime , info.time , info.fs);
% % axes(gca)
% % hold on
...
%%
% info.periodictime = segments2logical(testperenergytime , info.time , info.fs);
%%
...
wenergy = hamming_energy(weighted_bands, fs ,  0.02 , 1);
...
% %% Plotting for the persentation:
% tt = 1:length(xx);
% tt = (tt-1)/fs;
% 
% [m, mt] = findpeaks(wenergy);
% yes = m>high(mt);
% indicators = wenergy > low;
% 
% axes(ax);
% plot(tt, wenergy , 'b')
% hold on
% % plot(yin.time , periodictimeyin*10^-3 , 'k' , 'Linewidth' , 2)
% % plot(tt , high , 'g')
% plot(tt , indicators*10^-3 , 'r')
% plot(tt , detect*10^-3 , 'g--' , 'Linewidth' , 1.5)
% 
% plot(tt(mt(yes)) , m(yes) , 'o');
% plot(tt(mt(~yes)) , m(~yes) , 'x');
% 
% hold off
% legend('Local Energy', 'Low Threshold Indicator' , 'Detected' , 'Peak Detected' , 'Peak Rejected');
% 
% 
% disp('something')
